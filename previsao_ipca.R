...
# ano = as.numeric(format(data_proxima, "%Y"))
# mes = as.numeric(format(data_proxima, "%m"))
# dia = as.numeric(format(data_proxima, "%d"))
...
prev_desemp = forecast::forecast(desemprego_ts, h=6)%>%
  as.data.frame()%>%
...
prev_selic = forecast::forecast(selic_ts, h=6)%>%
  as.data.frame()%>%
  dplyr::select(`Point Forecast`)%>%
...
prev_cambio = forecast::forecast(cambio_ts, h=6)%>%
  as.data.frame()%>%
  dplyr::select(`Point Forecast`)%>%
...
# dados_juntar = dados%>%
#   dplyr::select(data, selic, cambio, desemprego, ipca)%>%
#   dplyr::select()
